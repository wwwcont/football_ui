    ...
    <div className="space-y-6">
      <section className="panel-matte rounded-2xl p-4">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-xs uppercase tracking-wide text-[#c7b37a]">Турнир</p>
        ...
        <div className="mt-4 grid grid-cols-3 gap-2">
          {[
            ...
            <div key={label} className="panel-soft rounded-xl p-2.5">
      ...
      <Section
        title="Live / Скоро"
        aside={<Link to="/matches" className="panel-soft rounded-full px-2.5 py-1 text-xs text-zinc-100">Все матчи</Link>}
      >
      ...
      <Section title="Топ-3 команды" aside={<Link to="/teams" className="panel-soft rounded-full px-2.5 py-1 text-xs uppercase text-zinc-100">Все команды</Link>}>
            ...
            return (
              <div key={entry.teamId} className="flex items-center justify-between px-3 py-2.5 last:border-none">
                <div className="flex items-center gap-2">
                  <span className="inline-flex h-5 w-5 items-center justify-center rounded-full border line-accent text-[11px] text-zinc-100">{entry.position}</span>
